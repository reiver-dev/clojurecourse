...
(defn parse-select-impl [query]
  (cm/match [query]
            ...
            [["where" field op val & rest]] (list (list :where (make-where-function field op val)) rest)
...
(defn parse-select [^String sel-string]
  (loop [input (vec (.split sel-string " "))
        query []]
    (if (empty? input)
      (let [res (apply concat query)]
        ;; Если получается пустой список - вернуть nil
        (if (empty? res) nil
            res))
