...
(defn handle-request [^Socket sock]
  (binding
      [*out* (java.io.OutputStreamWriter. (.getOutputStream sock))
       *in* (java.io.BufferedReader. (java.io.InputStreamReader. (.getInputStream sock)))] ;; переопределить *in* & *out* чтобы они указывали на входной и выходной потоки сокета
    (try
      (let [s (read-line)] ;; считать данные из переопределенного *in*
        (if (= (str/lower-case s) "quit")
          (deliver should-be-finished true);;; 1) сообщить основному потоку что мы завершаем выполнение.
               ...
               ;;;
          (println
           (pr-str (perform-query s)));;; 2) выполнить запрос при помощи perform-query и записать
